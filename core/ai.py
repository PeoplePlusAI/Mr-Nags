...
def bhashini_chat(chat_id, message):
    ...
    if status == "requires_action":
        if run:
            tools_to_call = run.required_action.submit_tools_outputs.tool_calls
        else:
            run, status = get_run_status(run, client, thread)
            tools_to_call = run.required_action.submit_tool_outputs.tools_calls
        ...
        for tool in tools_to_call:
            ...
            if func_name == "raise_complaint":
                ...
                if complaint:
                    ...
                    run = client.beta.threads.tuns.submit_tool_outputs(
                        thread_id = thread.id,
                        run_id = run.id,
                        tool_outputs = tool_output_array
                    )
